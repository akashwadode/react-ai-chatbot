...
def fetch_pending_notification():
    ...
    report details, and lab name. Ordered by oldest first.
    """
    conn = get_connection()
    cursor = conn.cursor()
    query = """
...
def fetch_all_pending_notifications():
    """
    Returns a list of all pending notifications (status=0) with patient details,
    report details, and lab name. Ordered by oldest first.
    """
    conn = get_connection()
    cursor = conn.cursor()
