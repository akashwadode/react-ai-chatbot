...
async def process_pending_notification():
    """
    Process one pending notification using the default template.
    """

    pending_list = fetch_pending_notifications(limit=1)
    ...
    signed_url = generate_signed_link(
        pid_hash,
        rid=str(report_id),
        expiry_minutes=1440
    )
...
async def process_pending_notification_new():
    """
    Process ALL pending notifications using the template
    'report_ready_notification_2'.

    Returns summary of successes and failures.
    """

    pending_list = fetch_pending_notifications()
    ...
    for pending in pending_list:
        ...
        result = send_whatsapp_new_template(
            patient_whatsapp,
            patient_name,
            lab_name,
            signed_url
